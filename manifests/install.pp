class s3fs::install {
  ...
  exec {'extract-s3fs':
    ...
    creates => "${s3fs::tarball_dir}/s3fs-${s3fs::s3fs_version}",
    require => Wget::Fetch['s3fs'],
  }
  exec {'autogen-configure-s3fs':
    cwd      => "${s3fs::tarball_dir}/s3fs-${s3fs::s3fs_version}/",
    provider => 'shell',
    command  => './autogen.sh',
    creates  => "${s3fs::tarball_dir}/s3fs-${s3fs::s3fs_version}/configure",
  ...
  exec {'configure-s3fs':
    ...
    creates     => "${s3fs::tarball_dir}/s3fs-${s3fs::s3fs_version}/Makefile",
